...
function buildTimelineOption(params: BuildScatterOptionParams) {
  ...
  const maxSeries = steps.reduce((m, s) => Math.max(m, s.series.length), 0)

  const frames = steps.map(step => ({
    series: [
      ...Array.from({ length: maxSeries }, (_, i) => {
        const s = step.series[i]
        const color = s?.color ?? clusterColor(i)
        return {
          name: s?.name ?? `Cluster ${i + 1}`,
  ...
  return {
    timeline: {
      ...
      label: {
        position: 12,
        formatter: (i: number) => steps[i]?.label ?? `step ${i}`,
        color: subtleColor,
      },
...
export function buildScatterOption(params: BuildScatterOptionParams) {
      ...
      ...(s.trendLine
        ? {
            markLine: {
              ...
              label: {
                formatter: s.trendLine.label ?? '',
                align: 'right' as const,
                color: labelColor,
              },
              tooltip: { formatter: s.trendLine.label ?? '' },
              data: [
                [
                  { coord: s.trendLine.from, symbol: 'none' },
                  { coord: s.trendLine.to, symbol: 'none' },
                ],
              ],
            },
          }
  ...
  const regressionList = series.flatMap((s, i) => {
    ...
    return [
      {
        name: `${s.name} (fit)`,
        type: 'line' as const,
        data: s.regression.points,
        smooth: s.regression.smooth ?? true,
        symbol: 'none' as const,
        z: 3,
  ...
  const singleGrid = {
    left:
      (legendShown && legendPosition === 'left' ? 96 : 12) + reserve('left'),
    right:
      (legendShown && legendPosition === 'right' ? 96 : 12) + reserve('right'),
    top:
      (legendShown && legendPosition === 'top' ? 40 : 12) +
      titleReserve +
      reserve('top'),
    bottom:
      (legendShown && legendPosition === 'bottom' ? 40 : 12) +
      reserve('bottom'),
    containLabel: true,
  }
  ...
  if (aggregate && view === 'bar') {
    ...
    const barSeries = series.map((s, i) => {
      const avg = s.data.length
        ? s.data.reduce((sum, p) => sum + p[dim], 0) / s.data.length
        : 0
      return {
        type: 'bar' as const,
        id: s.name,
        ...
        barCategoryGap: '40%',
        barMaxWidth: 80,
        itemStyle: { color: colorFor(s, i), borderRadius: [4, 4, 0, 0] },
        label: {
          show: true,
  ...
  return {
    ...
    xAxis: multiGrid ? panelGrids.map((_, i) => makeXAxis(i)) : makeXAxis(),
    yAxis: multiGrid ? panelGrids.map((_, i) => makeYAxis(i)) : makeYAxis(),
    tooltip: {
      show: showTooltip,
      trigger: 'item' as const,
      ...(showCrosshair
