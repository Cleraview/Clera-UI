...
export const Pie: React.FC<PieProps> = ({
  ...
  const allData = useMemo<PieDatum[]>(
    () => (series?.length ? series.flatMap(s => s.data) : data),
    [series, data]
  )

  const cardItems = useMemo(
    () => allData.filter(d => d.detail || d.richLabel),
    [allData]
  )
  ...
  const unselectAllBut = useCallback(
    (chart: ECharts, keepSeries: number | null) => {
      ...
      ;(option.series ?? []).forEach((s, seriesIndex) => {
        if (seriesIndex === keepSeries) return
        const length = Array.isArray(s.data) ? s.data.length : 0
        if (length > 0)
