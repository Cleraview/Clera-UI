...
// Center every story. `layout: 'centered'` can't be used because it shrink-wraps
// the story, collapsing the chart's `w-full` container to 0 width. Instead the
// story view gets a full-height flex box that centers the chart vertically and
// horizontally while keeping it sized and responsive; in docs we only constrain
// the width so examples still stack normally.
...
const lastPrice = intraday[intraday.length - 1][1]
const trendVariant = lastPrice >= PREV_CLOSE ? 'success' : 'destructive'

const timeOfDay = (v: string | number) =>
...
const dailyYear = buildDailyYear()

const dayMonth = (v: string | number) =>
  new Date(v).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })

const MINUTE = 60 * 1000
const MONITOR_START = new Date('2024-03-14T13:00:00').getTime()
...
const cpuSeries = buildMonitor(3, 38, 30)
const memSeries = buildMonitor(9, 52, 32)
...
// A small play-button icon (data URI) used to demonstrate rich x-axis labels.
const PLAY_ICON =
  'data:image/svg+xml,' +
  encodeURIComponent(
