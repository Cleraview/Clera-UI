...
export function buildLineMatrixOption(params: BuildLineMatrixOptionParams) {
  ...
  const rowItems: RowItem[] = rows.map(r =>
    typeof r === 'string'
      ? { label: r, divider: false }
      : { label: r.label, divider: Boolean(r.divider) }
  )
  ...
  cells.forEach((c, k) => {
    const id = `cell-${k}`
    const first = c.data.length ? valueOf(c.data[0]) : 0
    const last = c.data.length ? valueOf(c.data[c.data.length - 1]) : 0
    const net = last - first
    const color =
      c.color ??
      (c.variant
        ? resolveVariant(c.variant)
        : colorByTrend
          ? net >= 0
            ? up
            : down
  ...
  const yData = rowItems.map(r => ({ value: r.divider ? '' : r.label }))
