...
export function buildLinePanelsOption(params: BuildLinePanelsOptionParams) {
  ...
  const hasData = panels.some(p => p.series.some(s => s.data.length > 0))
  ...
  const anyGradient = panels.some(p =>
    p.series.some(s => (s.area ?? area) === 'gradient')
  )
  ...
  panels.forEach((panel, pi) => {
      ...
      axisLabel:
        xAxisPerPanel || isLast
          ? {
              ...
              hideOverlap: true,
              formatter:
                xAxisType === 'time' ? (v: number) => fmtX(v) : undefined,
            }
          : { show: false },
    ...
    panel.series.forEach(s => {
      ...
      series.push({
        ...
        symbol: 'circle',
        symbolSize: 6,
        ...(s.data.length > 200 ? { sampling: 'lttb' as const } : {}),
    ...
    tooltip: showTooltip
      ? {
          ...
          formatter: (
            ...
            const rows = arr
              .map(p => {
                const fmt = fmtBySeries[p.seriesName ?? ''] ?? formatValue
                return `${p.marker ?? ''}${p.seriesName}: ${fmt(valueOf(p.value))}`
              })
              .join('<br/>')
            return `${head}<br/>${rows}`
          },
