...
<DocsLayout>
  <div>
    ...
    To render grouped headings and separators, provide `options` with a `group` key and pass `groupBy="group"` to `ComboBox`.
    ...
    <Canvas>
      ...
      <Canvas.Source>
        <Source className="my-0!" dark code={`type ComboBoxOption = {
          value: string
        ...
        <ComboBox options={options} groupBy="group" />
        `} />
      </Canvas.Source>
    </Canvas>

    The visual separator between consecutive groups is inserted by the shared `BaseComboBox` implementation and uses the `styles.separator` class in [src/components/form/combobox/styles.ts](src/components/form/combobox/styles.ts).
