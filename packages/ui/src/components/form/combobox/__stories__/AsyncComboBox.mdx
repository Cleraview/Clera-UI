...
<DocsLayout>
  <div>
    ...
    ## **Grouping**
    `AsyncComboBox` expects `loadOptions` (and `loadDefaultOptions`) to return a grouped record where the keys are group titles and the values are arrays of option objects.

    <Canvas>
      <Canvas.Source>
        <Source className="my-0!" dark code={`
          type AsyncComboBoxOption = { value: string; label: React.ReactNode; [key: string]: unknown }
          type OptionGroups = Record<string, AsyncComboBoxOption[]>

          const loadOptions = async (search: string): Promise<OptionGroups> => {
            return {
              'Popular Products': [ { value: 'p1', label: 'Item 1' } ],
              'Categories': [ { value: 'c1', label: 'Category A' } ]
            }
          }
    ...
    The component will render each record key as a group heading and the group items beneath it. Separators between groups are automatically inserted by `BaseComboBox` using the `styles.separator` class.
