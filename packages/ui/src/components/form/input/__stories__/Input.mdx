...
<DocsLayout>
  <div>
    ...
    - Icon rendering: the visual icon inside inputs is owned by `FormInputWrapper`. Pass `icon` and `iconPosition` to `Input` and `MaskedInput`, and the wrapper handles spacing and label adjustments.
