...
<DocsLayout>
  <div>
    ...
    <DocInfo 
      importStatement={`import { AlertDialog } from '@clera-ui/alert-dialog'`}
    ...
    The `AlertDialog` component is used to present users with critical actions that require confirmation — such as deleting data, archiving projects, or performing irreversible operations. It ensures users make deliberate choices before continuing.
    ...
    <Canvas>
      ...
      <Canvas.Source>
        <Source className="my-0!" dark code={`
          import { AlertDialog } from '@clera-ui/alert-dialog'
