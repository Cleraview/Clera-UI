  ...
  return (
    <SectionShell direction="col" gapSize="md">
      <div className="relative flex-1 flex flex-col gap-space-lg md:gap-space-3xl">
        ...
        <div
          ref={navContainerRef}
          className="flex relative border-b-[1px] border-default overflow-x-auto md:overflow-visible opacity-0"
        >
          <div className="w-full flex justify-between">
            {advantages.map((item, index) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    'flex-1 relative p-space-md whitespace-nowrap transition-all duration-300 font-semibold cursor-pointer text-center border-b-[1px]',
                    isOpen
                      ? 'border-transparent'
                      : 'border-default text-subtlest hover:bg-white/5'
                  )}
                  onClick={() => setOpenIndex(index)}
                >
                  <h3 className={cn('text-heading-lg', isOpen && 'font-bold!')}>
                    {item.nav}
                  </h3>
                </div>
              )
            ...
            <div
              ref={indicatorRef}
              className="absolute bottom-0 h-[2px] bg-neutral-intense rounded transition-all"
          ...
          >
            <div className="flex-1 flex flex-col gap-space-md lg:gap-space-2xl">
              <div className="flex flex-col gap-space-sm">
                <h2 className="max-w-lg text-heading-2xl font-bold!">
                  {advantage.title}
                </h2>
