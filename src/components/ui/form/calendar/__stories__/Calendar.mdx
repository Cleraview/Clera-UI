...
<div className="flex gap-space-4xl">
  <div>
    ...
    <DocInfo 
      importStatement={`import Calendar from "@/components/ui/form/calendar"`} 
      sourcePath={`components/ui/form/calendar`} 
    />
    <br />
    ...
    Wrap the `Calendar` component in a `Form.Item` from `@/components/ui/form`. The `name` prop on `Form.Item` will link it to your `react-hook-form` state.
